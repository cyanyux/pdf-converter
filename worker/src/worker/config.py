...
ROOT = Path(os.environ.get("PDF_CONVERTER_ROOT", ".")).resolve()
DB_PATH = Path(os.environ.get("PDF_CONVERTER_DB", str(ROOT / "data/pdf-converter.db")))
SCHEMA_PATH = Path(os.environ.get("PDF_CONVERTER_SCHEMA", str(ROOT / "db/schema.sql")))
UPLOADS_DIR = Path(os.environ.get("PDF_CONVERTER_UPLOADS", str(ROOT / "data/uploads")))
OUTPUTS_DIR = Path(os.environ.get("PDF_CONVERTER_OUTPUTS", str(ROOT / "data/outputs")))

# 'cpu' | 'gpu' | 'gpu:0'
DEVICE = os.environ.get("PDF_CONVERTER_DEVICE", "gpu:0")
...
OCR_ENGINE = os.environ.get("PDF_CONVERTER_ENGINE", "onnxruntime").strip().lower()
...
ENABLE_HPI = _bool("PDF_CONVERTER_ENABLE_HPI", False)
# Searchable-PDF OCR engine: PP-OCRv6 is the 3.7 default; pin PP-OCRv5 for continuity.
OCR_VERSION = os.environ.get("PDF_CONVERTER_OCR_VERSION", "PP-OCRv6")
# Python-side page grouping for searchable-PDF OCR: pages are chunked this many per predict()
# call, bounding per-batch pixmap memory. This is NOT detection-level GPU batching (the PP-OCR
# pipeline still runs detection one page at a time), so raising it won't speed up detection.
OCR_BATCH_SIZE = _int("PDF_CONVERTER_BATCH", 4)
TEXT_REC_BATCH_SIZE = _int("PDF_CONVERTER_REC_BATCH", 16)
CPU_THREADS = _int("PDF_CONVERTER_CPU_THREADS", 8)
SEARCHABLE_DPI = _int("PDF_CONVERTER_DPI", 200)

# Cap VL input resolution to avoid native dynamic-graph VRAM spikes on dense pages.
VL_RENDER_ZOOM = float(os.environ.get("PDF_CONVERTER_VL_ZOOM", "1.5"))
VL_MAX_PIXELS = _int("PDF_CONVERTER_VL_MAX_PIXELS", 2200 * 2200)
...
VL_MAX_NEW_TOKENS = _int("PDF_CONVERTER_VL_MAX_NEW_TOKENS", 4096)
...
VL_REC_BACKEND = os.environ.get("PDF_CONVERTER_VL_REC_BACKEND") or None

POLL_S = _int("PDF_CONVERTER_POLL_MS", 200) / 1000
STALE_S = _int("PDF_CONVERTER_STALE_S", 120)
MAX_ATTEMPTS = _int("PDF_CONVERTER_MAX_ATTEMPTS", 3)
JOB_MAX_AGE_S = _int("PDF_CONVERTER_JOB_MAX_AGE", 7200)
GC_INTERVAL_S = _int("PDF_CONVERTER_GC_INTERVAL", 600)
...
JOB_IDLE_TIMEOUT_S = _int("PDF_CONVERTER_JOB_IDLE_TIMEOUT", 300)
# Post-recognition phases (VL table-merge/restructure, native docx save) do opaque
...
SAVE_IDLE_TIMEOUT_S = _int("PDF_CONVERTER_SAVE_IDLE_TIMEOUT", 1800)
...
WATCHDOG_TICK_S = _int("PDF_CONVERTER_WATCHDOG_TICK_MS", 2000) / 1000
...
MODEL_LOAD_TIMEOUT_S = _int("PDF_CONVERTER_MODEL_LOAD_TIMEOUT", 900)
...
REAP_INTERVAL_S = _int("PDF_CONVERTER_REAP_INTERVAL", 60)
...
CANCEL_GRACE_S = _int("PDF_CONVERTER_CANCEL_GRACE_S", 15)
